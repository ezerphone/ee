...
def main(page: ft.Page):
    ...
    # שינוי אייקון לרגיל (ללא ROUNDED)
    ...
    def stop_playback_timer():
        ...
        timer_running = False
        audio_player.pause()
        btn_play_pause.icon = ft.icons.PLAY_ARROW # אייקון רגיל
    ...
    def toggle_download(url, title, date, btn, p_bar, s_txt):
        if active_downloads.get(url):
            ...
            btn.icon = ft.icons.DOWNLOAD # אייקון רגיל
            ...
            btn.icon = ft.icons.CLOSE # אייקון רגיל
            btn.icon_color = "red"
            btn.update()
    ...
    def download_file_thread(url, title, date, btn, progress_bar, status_text):
        ...
        try:
            ...
            btn.icon = ft.icons.FOLDER_OPEN # אייקון רגיל
    ...
    def play_stream(url, title, is_live=False):
        ...
        btn_play_pause.icon = ft.icons.PAUSE # אייקון רגיל
    ...
    def toggle_play():
        if btn_play_pause.icon == ft.icons.PAUSE:
            audio_player.pause()
            ...
            for group in items_groups:
                for item in group.get('items', []):
                    ...
                    btn_dl = ft.IconButton(
                        icon=ft.icons.DOWNLOAD, # אייקון רגיל
                        icon_color="white",
                        tooltip="הורדה",
                    )
                    ...
                    card = ft.Container(
                        content=ft.Column([
                            ft.Row([
                                ft.IconButton(
                                    icon=ft.icons.PLAY_CIRCLE_FILLED, # אייקון רגיל
                                    icon_color=ft.colors.BLUE, 
                                    icon_size=34,
    ...
    def open_settings_dialog(e):
        ...
        def save_settings(e):
            ...
            if new_url:
                ...
                page.snack_bar = ft.SnackBar(ft.Text("הגדרות נשמרו. מרענן..."))
                page.snack_bar.open = True
                close_dlg(e)
        ...
        dlg_settings = ft.AlertDialog(
            ...
            content=ft.Column([
                ...
                txt_url_input,
                ft.Row([
                    ft.TextButton("אפס לברירת מחדל", on_click=reset_settings, style=ft.ButtonStyle(color=ft.colors.ORANGE)),
                ], alignment=ft.MainAxisAlignment.START),
                ft.Divider(),
                ft.Text(f"נתיב הורדה:\n{get_download_folder()}", size=11, color="grey"),
            ], height=250, width=320),
            actions=[
                ft.TextButton("ביטול", on_click=close_dlg, style=ft.ButtonStyle(color=ft.colors.RED)),
                ft.TextButton("שמור ורענן", on_click=save_settings, style=ft.ButtonStyle(color=ft.colors.GREEN)),
            ],
        )
